...
def creat_question():
    if request.method == "POST":
        ...
        add = Questions(question=question, subject_id=subject_id, levels_id=level_id)
        db.session.add(add)
        db.session.commit()
        add = Variants(subject_id=subject_id, levels_id=level_id, variants=variant)
        db.session.add(add)
        db.session.commit()
        return redirect(url_for("creat_question"))
    subject = Subject.query.all()
    levels = QuizLevels.query.all()
    return render_template("creat test.html", subject=subject, levels=levels)
